...
from blog.models import Category, Post, Menu, Page, Banner
...
def index(request):
    ...
    event_list = Event.objects.filter(is_active=True)[0:10]
...
def contact_us(request):
    ...
    if validate_contact.is_valid():
        ...
        new_message.save()
        data = {"data": 'Submitted successfully', "error": False}
        try:

            # send_mail(contact.name, contact.message, contact.email, [
            #           'shivkumarhegonde@gmail.com'], fail_silently=False)
...
def rss(request):
    ...
    xml_cont = '''<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
                        ...
                        <title>stmsco | blog</title>
                        <description>
                        stmsco blog and events
                        </description>
